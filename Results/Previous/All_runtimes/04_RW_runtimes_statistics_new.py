...
def filter_by_mod_zscore(values, threshold=20):
    """
...
RW_WO_percentages_n, RW_WO_percentages_a, RW_WO_percentages_b = {}, {}, {}
for name in graph_names:
    ...
    RW_WO_percentages_n[name] = df_n
    RW_WO_percentages_a[name] = df_a
    if name in baseline_graph_names:
        RW_WO_percentages_b[name] = df_b
...
with open(BASE / f"RW_WO_percentages_{threshold}_n.pkl", "wb") as f:
    pickle.dump(RW_WO_percentages_n, f)
    
with open(BASE / "RW_WO_percentages_{threshold}_a.pkl", "wb") as f:
    pickle.dump(RW_WO_percentages_a, f)
    
with open(BASE / "RW_WO_percentages_{threshold}_b.pkl", "wb") as f:
    pickle.dump(RW_WO_percentages_b, f)
